...
# Plot
ggplot(map_data_sf) +
  geom_sf(aes(fill = ratioMP), color = "black", size = 0.01) +
  scale_fill_viridis_c(
    name = "Pop : Provider Ratio",
    option = "inferno",
    trans = "log"  # Log scale emphasizes differences
  ) +
  labs(title = "Population to Mental Health Provider Ratio by County") +
  coord_sf(xlim = c(-125, -66), ylim = c(24, 50), expand = FALSE) +
  theme_minimal()
...
\[
\log(\mu_i) \;=\; \beta_0 + \beta_1\,\text{provider_ratio}_i
                 + \beta_2\,\text{loneliness}_i
                 + \beta_3\,\text{social_support}_i
                 + \beta_4\,\text{suicide_rate}_i ,
\]
...
When evaluating the 2 models, Random Forest and XGBoost, we saw that the RMSE value for XGBoost was 0.443 which was slightly lower than the RMSE for Random Forest, which was 0.449 This meant that the XGBoost model has a greater accuracy and predictive power compared to Random Forest. We also saw that in both models feelings of loneliness and social and emotional support were the 2 top features in the model.
